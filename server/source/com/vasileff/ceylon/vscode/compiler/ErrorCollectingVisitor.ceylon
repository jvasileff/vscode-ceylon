...
class ErrorCollectingVisitor(Backend backend) extends Visitor() {
    ...
    function forBackend(Tree.AnnotationList? annotations, Unit unit)
        =>  isForBackend(annotations, backend, unit)
    ...
    function forDart
        ...
        =>  if (exists unit = node.unit)
            then forBackend(annotationList(node), node.unit)
            else true;
